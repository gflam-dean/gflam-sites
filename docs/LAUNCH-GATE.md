...
| Anything opens a page and asserts what is on the screen | This was the biggest hole on the list and it is why a blank console and a flashing slide both reached Dean on the day this was written. | **partly**, and proved BOTH ways | `venueplay.com.au/screen-check?venue=<slug>` loads the venue screen in a frame and asserts what is actually PAINTED, not what is in the DOM: it is linked to a venue, something is on the wall, it says connected, it records which road it took, at most ONE six-character code is up (item 8 of the live-only list), a slide is lit, and waking it neither flashes the wall nor jumps back to slide one. Proved both ways on 10 Sep: `?venue=no-such-pub-anywhere` gives **1 of 10 FAILED**, a real venue gives **all 10 passed**. It is one HTML file on the same origin, so it runs on whatever device you open it on, INCLUDING THE FIRE STICK BEHIND THE BAR. **It watches for 40 seconds**, because a screen with a bad link paints a friendly holding page and only admits it at about 33s: two earlier versions finished in ten and passed a broken wall. What it still does not do: drive a game, click a host console, or hear a song. | Anyone, by opening it. The tab must be in front: a background tab throttles its timers. |
...
| A phone that loses wifi mid-question comes back | Forty people, one bad access point. | **yes** | Fixed 10 Sep, and it was worse than this row said. The phone did not handle CLOSED AT ALL (Supabase reports CHANNEL_ERROR, TIMED_OUT and CLOSED; it listed two), and it never cleared `subscribed` on any of them, so it kept writing answers into a dead channel while showing Connected. That is the CURRENT default road for every venue, not the room server. The handler was an anonymous callback inside ch.subscribe(), which is why no test could reach it; it is named now and `phone-reconnect.test.js` runs it against a fake channel and asserts what actually reached it. Proved by putting each half of the bug back: 6 of 22 red without CLOSED, 11 of 22 without the flag. | Already proved. Dean can still confirm with a phone in flight mode. |
...
| A Worker can be deployed while players are connected | Every deploy so far had been to an empty room. The first one that is not would be on a Friday night. | **yes** | Tested 10 Sep on Sydney staging with **186 players mid-game across 6 rooms**: the game Worker was deployed twice, and across the following minutes the canary held 157-204ms, the request rate never dipped, and the count of requests over one second did not move by a single one. A Cloudflare deploy is atomic and it was invisible. **The socket half took three passes to get right.** Room presence collapsed after a deploy (35 to 9, 31 to 15 in thirty seconds) and looked like a serious finding. Sampling the same rooms with NO deploy showed the same swing (32 to 5, back to 32 in forty-five seconds), so it could not be attributed to the deploy. Then an INDEPENDENT browser socket, nothing to do with the load tool, was held against the same Worker for **7.8 minutes sending every 25 seconds like a phone answering: one open, zero closes, zero errors**, while the load tool's rooms swung and recovered to full. The room server holds a socket; the swing was the harness's own client-side churn. | Already proved. Re-run the browser socket probe if the room server ever looks like it is dropping people. |
