...
| The live Workers are the Workers in the repo | A fix that was written, tested and never pasted is a fix that does not exist. | **no**, both Workers are behind | Live game Worker `57d50ed2`; the repo has phase 2 of the room server and is on `848abd2f`, which IS deployed to Sydney/staging and proved there. Live billing Worker `ab821ee7`; the repo is on `0e4d4af3` and carries the four idempotency keys and the webhook ledger, neither of which does anything until it is pasted. Migrations 78 and 79 are already on both databases, and both Workers treat a missing table as "old behaviour", so the order is safe. | Dean, by pasting both, then `python3 tools/release-check.py`, which asks each Worker its own build. |
...
| How many venues can play at once | This is the number that decides whether there is a business, and it is not the number anyone assumed. | **measured, and phase 2 moved it** | On the free tier, 10 Sep: 6 trivia rooms healthy at 89 ms, 15 trivia rooms COLLAPSED to a 19 second TV poll, 35 bingo rooms healthy at 84 ms. Trivia is the format that runs out first, because thirty phones answering every 25 seconds is thirty writes, and it is also the only format Queensland is open to today. With the room server holding answers, the same 15 trivia rooms went to **199 ms phone answers and 215 ms TV polls**, from 18,329 ms and 14,378 ms. The other ceiling is Realtime connections, 200 on free, which is about FOUR venues whatever the database does; the room server removes that one too, because a room is a Cloudflare socket and not a Supabase one. | Measured with `game-load.py` against Sydney. Still to prove: the same numbers with the room server as the DEFAULT rather than opt-in, and on Pro. |
